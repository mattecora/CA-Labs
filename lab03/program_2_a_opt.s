...
; Used registers:
; - r1: loop accumulator for loads (starting from the end)
; - r2: loop accumulator for stores (starting from the end)
...
main:   daddi   r1, r0, 232             ; load r1
        daddi   r2, r0, 240             ; load r2

        l.d     f1, v1(r1)              ; preload f1
        l.d     f2, v2(r1)              ; preload f2

loop:   mul.d   f3, f1, f2              ; f3 = f1 * f2
        
        daddi   r1, r1, -8              ; decrement r1
        daddi   r2, r2, -8              ; decrement r2
        ...
        bnez    r2, loop                ; loop if r2 != 0

        halt

; 819 cycles, 873 RAW, 60 struct