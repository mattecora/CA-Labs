...
; Used registers:
; - r1: loop accumulator (starting from the end)
; - f1, f2: temporary storage for operands from v1, v2
; - f3, f4: temporary storage for the results of mul and div

; Algorithm:
; - Load the registers f1, f2 with values from v1, v2
; - Perform the required operations saving the results in f3 and f4
; - Store the values of f3, f1, f5 into v3, v4, v5
; - Loop for 30 times
...
loop:   daddi   r1, r1, -8              ; decrement r1

        l.d     f1, v1(r1)              ; load f1
        l.d     f2, v2(r1)              ; load f2

        s.d     f1, v4(r1)              ; store v4
