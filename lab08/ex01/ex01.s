...
__initial_psp   SPACE   Stack_Size/2
__initial_msp
...
Reset_Handler   PROC
				...
				; your code here
                MOV     R0, #3
                MSR     CONTROL, R0             ; Go to user mode
                LDR     SP, =__initial_psp      ; Load initial stack address
                
                LDR     R2, =0xFFFFFFFF                         
                SVC     0x2                     ; Run SVC to reset R2
                
                LDR     R6, =0x6
                SVC     0x6                     ; Run SVC to reset R6
                
                LDR     R12, =0xC
                SVC     0xC                     ; Run SVC to reset R12
                
                SVC     0x1B                    ; Run SVC with no effect
                
                LDR     R1, =17
                LDR     R2, =3
                SVC     0x51                    ; Run SVC to compute R0 = R1 % R2
...
;               0 <= SVC <= 12
                ...
                CMP     R0, #3
                STRLS   R2, [R1, R0, LSL #2]    ; If R0-R3, also update stack frame in PSP
                BLS     End_SVC_Handler
                
                STR     R2, [SP, R0, LSL #2]    ; Otherwise, use the MSP
                BLS     End_SVC_Handler
...
;               SVC >= 64
                ...
                AND     R3, R0, #0x38           
                LSR     R3, R3, #3              ; Second register index in R3
                LDR     R3, [SP, R3, LSL #2]    ; Divisor in R3
                
                UDIV    R4, R2, R3              ; R4 = R2 / R3
